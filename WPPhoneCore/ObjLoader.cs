...
public class ObjMeshLoader
{
	public static StreamReader[] LoadMeshes(string fileName) {
		...
		StreamReader mreader = new StreamReader(Microsoft.Xna.Framework.TitleContainer.OpenStream(fileName));
	...
	StreamReader mreader = new StreamReader(TitleContainer.OpenStream(path));
		string current = "";
		bool isfound = false;
		while(!mreader.EndOfStream) {
